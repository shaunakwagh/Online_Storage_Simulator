...
int head1 =0;
struct node_seq *head = NULL;
struct node_seq *current = NULL;
...
int getSeq(SG_Node_ID nodeId){
   ...
   struct node_seq *ptr = head;
    
   //start from the beginning
   while(ptr != NULL) {
      if (ptr->nodeId == nodeId){
        printf("SEARCH NODE IS %d, SEQ IS %d\n", ptr->nodeId , ptr->seqNo);
        return(ptr->seqNo); 
      }
   }
}
...
void update_node_seq(SG_Node_ID nodeId, int seqNo) {
   ...
   while(ptr != NULL) {
      if (ptr->nodeId = nodeId){
        ...
        update=1;
        printf("BEFORE BREAK\n");
        printf("UPDATE NODE IS %d SEQ is %d\n", ptr->nodeId, ptr->seqNo );
        break;
      }
      ptr = ptr->next;
   }
...
void insertNodeSeq(SG_Node_ID nodeId, int seqNo) {
    ...
    struct node_seq *ptr = head;
   //create a link
   struct node_seq *link = (struct node_seq*) malloc(sizeof(struct node_seq));
    
   link->nodeId = nodeId;
   link->seqNo = seqNo;
   printf("INSERT NODE IS %d SEQ is %d\n", link->nodeId, link->seqNo );
   ...
   /*
   printf("AAAAA head1 is %d\n", head1);
   if ( head1 == 1 ){
    printf("TTTTTTT links is %d\n", link);
    ptr->next = link;
    printf("XXXXXX");
   }
   else {
...
printf("\n\n****   R E A D rcount is %d *****  \n\n", ++rcount);
printf("R E A D END node is %d, seq is %d\n", fs[fh].remid, fs[fh].remseqen);
...
//memcpy(tbuf, buf, 1024);
 ...
 //else {
      /*fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] = fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] + 1;*/
      int seqNo = getSeq(fs[fh].remid);
      seqNo++;
      fs[fh].remseqen++;
      if ( (ret = serialize_sg_packet( SG_NODE_UNKNOWN, // Local ID
                                    ...
                                    seqNo,  // Receiver sequence number
...
//printf("r 333 pktlen is %d rpktlen is %d\n\n", pktlen, rpktlen);
    ...
    update_node_seq(rem, srem);
    //fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] = srem;
    //fs[fh].remseqen = srem;
...
{
  memcpy(buf,tbuf,256);
  printf("\n## READ  first 256 of buf ###\n");

  //fwrite(buf, 1, 512, stdout);
//printf("\n\n");
}
if ((fs[fh].fp % 1024) ==256)
{
printf("\n## READ second 256 of buf ###\n");

 memcpy(buf,tbuf+256,256);
 //fwrite(buf, 513, 1024, stdout);
//printf("\n\n");
}
...
if ((fs[fh].fp % 1024) == 512)
{
    printf("\n## READ third 256 of buf ###\n");
  memcpy(buf,tbuf+512,256);
// fwrite(buf, 513, 768, stdout);
//printf("\n\n");
}

if ((fs[fh].fp % 1024) == 768)
{
    printf("\n## READ fourth 256 of buf ###\n");
  memcpy(buf,tbuf+768,256);
 
}


/*if (!cbuf){
        printf("1 P U T I N C A C H E\n");
...
*/
    ...
    //printf("W R I T E BEGIN node is %d, seq is %d\n", fs[fh].remid, fs[fh].remseqen);
   ...
   // sgLocalSeqno = SG_INITIAL_SEQNO;
    ...
    if ((fs[fh].fp / 256) % 4 == 0)
    {
         //printf("\n IN Written BEFORE rem seqno is %d\n", fs[fh].remseqen);

        //printf("111\n");
        // Serialize the buffer
            
            if (fs[fh].fp >= fs[fh].size)
            {
                //fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] = fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] + 1;
                if ((ret = serialize_sg_packet(SG_NODE_UNKNOWN, // Local ID
                        ...
                        sgLocalSeqno++, // Sender sequence number
                        SG_NODE_UNKNOWN,
                        // fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE],    // Receiver sequence number
                ...
                if ((ret = deserialize_sg_packet(&loc, &rem, &blkid, &op, &sloc, &srem, NULL, rbuf, rpktlen)) != SG_PACKT_OK)
                {
                    //printf("ret is %d\n",ret);
                ...
                update_node_seq(rem, srem);
                printf("1 D-SER NODE IS %d SEQ is %d\n", rem, srem);
                //fs[fh].remseqen = srem;
               //fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] = srem;
                //printf("\n 111.444 sgLocalSeqno is %d\n", sgLocalSeqno);
    ...
    //else {
                ...
                //fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] = fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] + 1;
            //fs[fh].remseqen++;
              seqNo = getSeq(fs[fh].remid);
              seqNo++;
              printf("1 SER NODE IS %d SEQ is %d\n", fs[fh].remid, seqNo);
                //printf("OUR SEQ is %d\n", fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE]);
                if ((ret = serialize_sg_packet(SG_NODE_UNKNOWN, // Local ID
                        fs[fh].remid,
                        //fs[fh].remid[fs[fh].fp / SG_BLOCK_SIZE],    // Remote ID
                        fs[fh].blkid[fs[fh].fp / SG_BLOCK_SIZE],    // Block ID
                ...
                if ((ret = deserialize_sg_packet(&loc, &rem, &blkid, &op, &sloc, &srem, tbuf, rbuf, rpktlen)) != SG_PACKT_OK)
                {
                ...
                update_node_seq(rem,srem);
                printf("2 D-SER NODE IS %d SEQ is %d\n", rem, srem);
                //fs[fh].remseqen = srem;
                //fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] = srem;
                //printf("AFTER DESER SEQ is %d\n", fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE]);
                /*
                if (loc == SG_NODE_UNKNOWN)
                {
                    logMessage(LOG_ERROR_LEVEL, "sgwrite: bad local ID returned[%ul]", loc);
                ...
                */
                ...
                if ((fs[fh].fp % 1024) ==256)
                {
                printf("\n## WRITE/UPDATE second 256 of buf ###\n");

                 memcpy(tbuf+256,buf,256);
                 //fwrite(buf, 513, 1024, stdout);
                //printf("\n\n");
                }
                ...
                if ((fs[fh].fp % 1024) == 512)
                {
                    printf("\n## WRITE/UPDATE third 256 of buf ###\n");
                  memcpy(tbuf+512,buf,256);
                 //fwrite(buf, 513, 768, stdout);
                //printf("\n\n");
                }
                ...
                if ((fs[fh].fp % 1024) == 768)
                {
                    printf("\n## WRITE/UPDATE fourth 256 of buf ###\n");
                  memcpy(tbuf+768,buf,256);
                 }
         ...
         //}
                ...
                // Update
                 ...
                 seqNo = getSeq(fs[fh].remid);
                 seqNo++;
                 printf("2 SER NODE IS %d SEQ is %d\n", fs[fh].remid, seqNo);
                //fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] = fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] + 1;
                if ((ret = serialize_sg_packet(SG_NODE_UNKNOWN, // Local ID
                        fs[fh].remid, //[fs[fh].fp / SG_BLOCK_SIZE],    // Remote ID
                        fs[fh].blkid[fs[fh].fp / SG_BLOCK_SIZE],    // Block ID
                        ...
                        sgLocalSeqno++, // Sender sequence number
                        seqNo, // [fs[fh].fp / SG_BLOCK_SIZE],   // Receiver sequence number
                ...
                update_node_seq(rem,srem);
                printf("3 D-SER NODE IS %d SEQ is %d\n", rem, srem);
                //fs[fh].remseqen = srem;
                ...
                /*if (!cbuf){
                    printf("P U T I N C A C H E\n");
                    putSGDataBlock(rem, blkid, tbuf);
                }*/
            }
          //printf("\n IN WRITE 222.555 srem seqno is %d\n", srem);
         
         //printf("\n IN WRITE 2226666 rem seqno is %d\n", fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE]);
         fs[fh].remid = rem;
        //fs[fh].remid[fs[fh].fp / SG_BLOCK_SIZE] = rem;  //Remote Node ID
          //printf("\n IN WRITE 2227777 NODEis %d\n", rem);
        ...
        printf("W R I T E END  1 node is %d, seq is %d\n", fs[fh].remid, fs[fh].remseqen);
        return (len);
  }
...
if (cbuf != NULL){
    ...
    memcpy(tbuf+512,cbuf+512,512);

  
}//@@@*/
//else {
        //printf("\n IN UPDATE 222.555 rem seqno is %d\n", srem);
        //fs[fh].remseqen++;
         //printf("\n IN UPDATE 222.666 rem seqno is %d\n", fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] );
         //fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] = fs[fh].remseqen[fs[fh].fp / SG_BLOCK_SIZE] + 1;

         //printf("\n IN UPDATE 222.777  NODE is %d\n", fs[fh].remid[fs[fh].fp / SG_BLOCK_SIZE] );

        seqNo = getSeq(fs[fh].remid);
        seqNo++;
        printf("3 SER NODE IS %d SEQ is %d\n", fs[fh].remid, seqNo);
        if ((ret = serialize_sg_packet(SG_NODE_UNKNOWN, // Local ID
                fs[fh].remid,  //[fs[fh].fp / SG_BLOCK_SIZE],    // Remote ID
                fs[fh].blkid[fs[fh].fp / SG_BLOCK_SIZE],    // Block ID
                SG_OBTAIN_BLOCK,    // Operation
        ...
        update_node_seq(rem,srem);
        printf("4 D-SER NODE IS %d SEQ is %d\n", rem, srem);
        //fs[fh].remseqen = srem;
        //printf("333.222\n");
        // Sanity check the return value
        /*if (loc == SG_NODE_UNKNOWN)
        ...
        // printf("\n IN UPDATE 222.333 loc seqno is %d\n", sloc);
         //   printf("\n N UPDATE 222.444 sgLocalSeqno is %d\n", sgLocalSeqno);
        ...
        else if ( (fs[fh].fp %1024) == 512)
         {
            memcpy(tbuf + 512, buf, 256);
        }
        else if ( (fs[fh].fp%1024) == 768)
        {
             memcpy(tbuf + 768, buf, 256);
        }
 ...
 //}
        ...
        seqNo = getSeq(fs[fh].remid);
        seqNo++;
        printf("4 SER NODE IS %d SEQ is %d\n", fs[fh].remid, seqNo);
        if ((ret = serialize_sg_packet(SG_NODE_UNKNOWN, // Local ID
                fs[fh].remid, //[fs[fh].fp / SG_BLOCK_SIZE],    // Remote ID
                fs[fh].blkid[fs[fh].fp / SG_BLOCK_SIZE],    // Block ID
                SG_UPDATE_BLOCK,    // Operation
 ...
 */
        ...
        update_node_seq(rem,srem);
        printf("5 D-SER NODE IS %d SEQ is %d\n", rem, srem);
        //fs[fh].remseqen = srem;
        //printf("555.222\n");
        // Sanity check the return value
       /* if (loc == SG_NODE_UNKNOWN)
        ...
        fs[fh].remid = rem;  //Remote Node ID
        fs[fh].blkid[fs[fh].fp / SG_BLOCK_SIZE] = blkid;    //Block Id
        ...
        printf("W R I T E END node is %d, seq is %d\n", fs[fh].remid, fs[fh].remseqen);
        return (len);
    }
}
////////////////////////////////////////////////////////////////////////////////
...
int sgshutdown(void) {
  // Local variables
    ...
    if ( (ret = serialize_sg_packet( SG_NODE_UNKNOWN, // Local ID
                                    ...
                                    sgLocalSeqno++,    // Sender sequence number
                                    SG_NODE_UNKNOWN,  // Receiver sequence number
