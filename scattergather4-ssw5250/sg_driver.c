...
*/
    ...
    if ((fs[fh].fp / 512) % 2 == 0)
    {
        ...
        if (fs[fh].fp >= fs[fh].size)
        {
            ...
            if ((ret = deserialize_sg_packet(&sgLocalSeqno, &rem, &blkid, &op, &sloc, &srem, NULL, rbuf, rpktlen)) != SG_PACKT_OK)
            {
                //printf("ret is %d\n",ret);
...
if (cbuf != NULL){
    ...
    memcpy(tbuf,cbuf,512);
            ...
            if ((ret = deserialize_sg_packet(&sgLocalSeqno, &rem, &blkid, &op, &sloc, &srem, tbuf, rbuf, rpktlen)) != SG_PACKT_OK)
            {
